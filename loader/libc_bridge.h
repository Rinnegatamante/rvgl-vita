...
int sceLibcBridge_fputs(char *string, FILE *fp);
char *sceLibcBridge_fgets(char *string, int len, FILE *fp);

int sceLibcBridge_fgetpos(FILE *stream, void *pos);
int sceLibcBridge_fsetpos(FILE *stream, const void *pos);

int sceLibcBridge_setjmp(jmp_buf env);
void sceLibcBridge_longjmp(jmp_buf env, int val);
void sceLibcBridge_rewind(FILE *fp);
